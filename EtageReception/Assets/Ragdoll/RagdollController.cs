...
public class RagdollController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (InputManager.GetButtonDown("TrunkUp") || GameTools.OnceIfTrue(80085, InputManager.GetAxis("TrunkUp") > 0.5f))
        {
            Straighten();
        }
        if (InputManager.GetButtonDown("TrunkDown") || GameTools.OnceIfTrue(80086, InputManager.GetAxis("TrunkDown") > 0.5f))
            Flacidify();
        if (InputManager.GetAxis("TrunkUp") < 0.5f && InputManager.GetAxis("TrunkDown") < 0.5f && !(InputManager.GetButton("TrunkDown") || InputManager.GetButton("TrunkUp")))
            UnStraighten();
        ...
        if (_isGrabbing == false)
        {
            ...
            if (_objectGrabbed != null)
            {
                if (_objectGrabbed.GetComponent<ConfigurableJoint>() != null)
                {
                    ...
                    {
                        if(_objectGrabbed.GetComponent<ChangeMusic>() == null)
                        {
                            AudioManager.Instance.StartSpecialSound(_objectGrabbed.GetComponent<SpecialSound>().Sound);
                        }
                        else
                        {
                            _objectGrabbed.GetComponent<ChangeMusic>().MusicChange();
                        }
                    }
                    else
                    {
